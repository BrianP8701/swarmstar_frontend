...
def get_kv(db_name: str, collection_name: str, _id: str) -> dict:
    ...
    try:
        ...
        if result is None:
            raise ValueError(
                f"_id {_id} not found in MongoDB collection {collection_name}."
            )
        id = result.pop("_id")
        result["id"] = id
        return result
...
def update_kv(
    db_name: str, collection_name: str, _id: str, updated_values: dict
) -> None:
    """
    ...
    try:
        ...
        for attempt in range(retries):
            ...
            if result.matched_count:
                ...
                raise Exception(
                    "Failed to update document due to concurrent modification."
                )
...
def set_kv(db_name: str, collection_name: str, _id: str, new_value: dict) -> None:
    ...
    try:
        ...
        for attempt in range(retries):
            ...
            new_document = (
                new_value.copy()
            )  # Make a copy to avoid mutating the original argument
            new_document["version"] = new_version

            # Attempt to replace the document if the version hasn't changed
            result = collection.replace_one(
                {"_id": _id, "version": current_document["version"]}, new_document
            )
            ...
            if result.matched_count:
                ...
                raise Exception(
                    "Failed to replace document due to concurrent modification."
                )
            # If the document was updated elsewhere, retry the operation
    except Exception as e:
...
def append_to_list(
    db_name: str, collection_name: str, _id: str, key: str, value
) -> None:
    """
    ...
    try:
        ...
        raise ValueError(
            f"Failed to append value {value} to list in MongoDB collection {collection_name}: {str(e)}"
        )


def remove_from_list_by_value(
    db_name: str, collection_name: str, _id: str, key: str, value
) -> None:
    """
    ...
    try:
        ...
        if collection.count_documents({"_id": _id}) == 0:
            raise ValueError(
                f"Document with _id {_id} not found in MongoDB collection {collection_name}."
            )
        ...
        if result.modified_count == 0:
            raise ValueError(
                f"Value {value} not found in list {key} in MongoDB collection {collection_name} at _id {_id}."
            )

    except Exception as e:
        raise ValueError(
            f"Failed to remove value {value} from list in MongoDB collection {collection_name}: {str(e)}"
        )


def get_element_by_index(
    db_name: str, collection_name: str, _id: str, index: int
) -> any:
    """
    ...
    try:
        ...
        if result and "data" in result and len(result["data"]) > 0:
            ...
            raise ValueError(
                f"Element at index {index} not found in MongoDB collection {collection_name} at _id {_id}."
            )
    except Exception as e:
...
def get_list_length(db_name: str, collection_name: str, _id: str) -> int:
    ...
    try:
        ...
        if result and len(result) > 0:
            ...
            raise ValueError(
                f"_id {_id} not found or list is empty in MongoDB collection {collection_name}."
            )
    except Exception as e:
        raise ValueError(f"Failed to get list length: {str(e)}")


def does_kv_exist(db_name: str, collection_name: str, _id: str) -> bool:
    """
    ...
    try:
        ...
        raise ValueError(
            f"Failed to check if _id {_id} exists in MongoDB collection {collection_name}: {str(e)}"
        )
